...
private:
    ...
    EventLoop *loop_ = nullptr;        // loop和Channel是一对一的关系
...
public:
    ...
    void handleevent();  // 处理epoll_wait返回的事件

    void newconnetion(Socket *servsock);  // 用于处理客户端的连接请求
    void onmessage();     // 用于处理对端发过来的报文
